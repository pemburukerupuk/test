		...
		if($password == $c_password){
				...
				if(isset($_POST['ubah_email'])){
					if($row['email'] == $emailbaru){
							...
							$sql2 = $con->query("UPDATE user SET username='$username', email='$emailbaru', password='$hash_password', update_at=NOW() WHERE u_id='$u_id' ");
							if($sql2 == 1){
								echo "<script>
					...
					$sql1 = $con->query("UPDATE user SET username='$username', password='$hash_password', update_at=NOW() WHERE u_id='$u_id' ");
